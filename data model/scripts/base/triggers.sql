...
begin
	if tg_op = 'INSERT' then
		...
		perform * from language_attribute where (language_code, name_, lang) = (new.language_code, new.name_, new.lang) and until > new.since and since < new.until
			and (language_code, name_, lang, since, until) != (old.language_code, old.name_, old.lang, old.since, old.until)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (language_code=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') beeing inserted (or updated) into LANGUAGE_ATTRIBUTE overlaps with a period of another value of the attribute.',
...
begin
	if tg_op = 'INSERT' then
		...
		perform * from country_attribute where (country_code, name_, lang) = (new.country_code, new.name_, new.lang) and until > new.since and since < new.until
			and (country_code, name_, lang, since, until) != (old.country_code, old.name_, old.lang, old.since, old.until)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (country_code=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') beeing inserted (or updated) into COUNTRY_ATTRIBUTE overlaps with a period of another value of the attribute.',
