...
begin
	if tg_op = 'INSERT' then
		...
		perform * from group_kind_attribute where (group_kind_code, name_, lang) = (new.group_kind_code, new.name_, new.lang) and until > new.since and since < new.until
			and (group_kind_code, name_, lang, since) != (old.group_kind_code, old.name_, old.lang, old.since)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (group_kind_code=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') being inserted (or updated) into GROUP_KIND_ATTRIBUTE overlaps with a period of another value of the attribute.',
...
begin
	if tg_op = 'INSERT' then
		...
		perform * from group_attribute where (group_id, name_, lang) = (new.group_id, new.name_, new.lang) and until > new.since and since < new.until
			and (group_id, name_, lang, since) != (old.group_id, old.name_, old.lang, old.since)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (group_id=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') being inserted (or updated) into GROUP_ATTRIBUTE overlaps with a period of another value of the attribute.',
...
begin
	if tg_op = 'INSERT' then
		...
		perform * from role_attribute where (role_code, name_, lang) = (new.role_code, new.name_, new.lang) and until > new.since and since < new.until
			and (role_code, name_, lang, since) != (old.role_code, old.name_, old.lang, old.since)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (role_code=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') being inserted (or updated) into ROLE_ATTRIBUTE overlaps with a period of another value of the attribute.',
...
begin
	if tg_op = 'INSERT' then
		...
		perform * from party_attribute where (party_id, name_, lang) = (new.party_id, new.name_, new.lang) and until > new.since and since < new.until
			and (party_id, name_, lang, since) != (old.party_id, old.name_, old.lang, old.since)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (party_id=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') being inserted (or updated) into PARTY_ATTRIBUTE overlaps with a period of another value of the attribute.',
...
declare
	l_since timestamp;
begin
	select until into l_since from group_attribute where group_id = a_group_id and name_ = a_column_name and lang = '-' order by until desc limit 1;
...
begin
	...
	if new.subgroup_of is distinct from old.subgroup_of then perform group_archive_value(old.id, 'subgroup_of', cast(old.subgroup_of as varchar)); end if;
	return new;
end; $$ language plpgsql;
...
declare
	l_since timestamp;
begin
	select until into l_since from party_attribute where party_id = a_party_id and name_ = a_column_name and lang = '-' order by until desc limit 1;
	if not found then l_since = '-infinity'; end if;
