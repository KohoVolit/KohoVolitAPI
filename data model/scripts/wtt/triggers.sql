...
begin
	if tg_op = 'INSERT' then
		...
		perform * from letter_attribute where (letter_id, name_, lang) = (new.letter_id, new.name_, new.lang) and until > new.since and since < new.until
			and (letter_id, name_, lang, since, until) != (old.letter_id, old.name_, old.lang, old.since, old.until)
			limit 1;
	end if;
	if found then
		raise exception 'Time period in the row (letter_id=%, name_=''%'', value_=''%'', lang=''%'', since=''%'', until=''%'') beeing inserted (or updated) into lETTER_ATTRIBUTE overlaps with a period of another value of the attribute.',
