...
class UpdaterSkStarostovia
{
	...
	/// id of the group for this parliament in the current term
	private $parliament_group_id;

	/// an array to mark all found existing memeberships to close the remaining ones
	private $marked;
	 ...
	 *
	 * If parameter <em>$param['area']</em> is set, constituency areas are updated as well.
	 *
	...
	*/
	private function updateMembership($mp_id, $town)
	{
		...
		$membership = array(
			'mp_id' => $mp_id,
			'group_id' => $this->parliament_group_id,
			'role_code' => 'member',
		...
		if(!$membership_db) { //insert
			...
			$this->log->write('Inserted a new membership: ' . $mp_id . ', ' . $this->parliament_group_id);
		}
		$this->marked[$mp_id][$this->parliament_group_id]['member'] = true;
	}
	...
	*/
	private function closeMemberships()
	{
		...
		$mps_db = $this->api->read('MpInGroup', array('group_id' => $this->parliament_group_id, 'role_code' => 'member', '_datetime' => 'now'));
			...
			if (!$other_mp)
				...
				// if conflict_mps indicates that this MP is already in the database, update his data and insert his source code for this parliament
					if (!empty($this->conflict_mps[$src_code])) {
						...
						// if null is given instead of an existing MP in database, insert MP as a new one, insert his source code for this parliament and generate a value for his disambiguation column
	...
	*/
	private function updateAreas($areas)
	{
		...
		foreach ($areas['list'] as $area) {
			...
			$this->log->write('Inserted a new area: ' . $ar['locality']);
		}
	}
	...
	*/
	private function updateConstituencies($towns)
	{
		...
		foreach ($towns['list'] as $town) {
			...
			$this->log->write('Inserted a new constituency ' . $ar['name']);
		}
	}
	...
	private function updateTermsAndParliament()
	{
		...
		foreach ($terms as $term) {
			...
			if ($term_db) { //update
				...
				$this->api->update('Term', array('id' => $term_id), $term);
			} else { //insert
		...
		if ($parliament_db) { //update
			...
			$this->api->create('ParliamentAttribute', array(
				array('parliament_code' => 'sk/starostovia', 'lang' => 'en', 'name' => 'name', 'value' => 'Mayors'),
				array('parliament_code' => 'sk/starostovia', 'lang' => 'en', 'name' => 'short_name', 'value' => 'Mayors'),
				array('parliament_code' => 'sk/starostovia', 'lang' => 'en', 'name' => 'description', 'value' => 'Mayors of towns and cities.')
			));

			// a function to show appropriate info about representatives of this parliament for use by NapisteJim application
			$this->api->create('ParliamentAttribute', array('parliament_code' => 'sk/starostovia', 'name' => 'napistejim_repinfo_function', 'value' => 'napistejim_repinfo_politgroup'));
		}
		...
		$group_db = $this->api->readOne('Group', array('group_kind_code' => 'parliament', 'parliament_code' => 'sk/starostovia', 'term_id' => $term_id));
		if ($group_db) { //update
			$this->log->write('Updating group: ' . $parl_group['name']);
			$this->api->update('Group', array('id' => $group_db['id']), $parl_group);
			$this->parliament_group_id = $group_db['id'];
		} else { //insert
			$this->log->write('Inserting a new group: ' . $parl_group['name']);
			$g = $this->api->create('Group', $parl_group);
			$this->parliament_group_id = $g['id'];
		}
